...
client(){
    ...
    numactl --cpubind=0 ib_send_lat  -d ${device} -a -n ${number_iterations} -R -F -I ${inline_size} ${server_ip} --perform_warm_up | grep -v "^ local" | grep -v "^ remote" | tail -n +17 |sed 's/\s\+/,/g' | sed 's/-\+//g' | sed "s/^/${experiment},lat,${device},${protocol},${inline_size},${number_iterations},${server},${client}/" | head -n -1 | tee -a $FILE
    exit 1
}
